...
FIXED water.py
=========================================
WATER XML DATAFRAME / GEOJSON → GRAPH CREATION + VISUALIZATION
Supports:
1. GeoJSON pipeline input
...
class WaterGraphBuilder:
    def __init__(self):
        ...
        print(f"Water graph output directory: {self.output_dir}")
    ...
    # =====================================
    # BUILD GRAPH FROM GEOJSON PIPELINES
    # =====================================
            ...
            for coord in coordinates:
                ...
                if previous_node:
                    ...
                    G.add_edge(
                        ...
                        current_node,
                        weight=distance,
                        pipeline_name=properties.get("name", "N/A"),
                        pipeline_type=properties.get("type", "N/A")
                    )
    ...
    # =====================================
    # BUILD GRAPH FROM WATER XML DATAFRAME
    # =====================================
    def build_graph_from_dataframe(self, df):
        try:
            ...
            for index, row in df.iterrows():
                ...
                node_id = str(row["node_ref"]).split(",")[0]

                G.add_node(
                    node_id,
                    latitude=lat,
                    longitude=lon,
                    water_type=row.get("type", "N/A"),
                    name=row.get("name", "N/A"),
                    operator=row.get("operator", "N/A"),
                    landuse=row.get("landuse", "N/A"),
                    man_made=row.get("man_made", "N/A")
                )

                # Sequential logical connection
                if previous_node:
                    prev_coord = (
                        G.nodes[previous_node]["latitude"],
                        G.nodes[previous_node]["longitude"]
            ...
            if G.number_of_nodes() == 0:
                print("No valid water nodes found.")
                return None
            ...
            print(f"Error building water graph from dataframe: {e}")
            return None
    ...
    def save_graph(self, G):
        try:
            graph_path = self.output_dir / "water_network.graphml"

            nx.write_graphml(G, graph_path)

            print(f"Water GraphML saved at: {graph_path}")
    ...
    def visualize_interactive_map(self, G):
        try:
            ...
            for node, data in G.nodes(data=True):
                popup_text = f"{data.get('name', 'N/A')} ({data.get('water_type', 'N/A')})"
            ...
            map_path = self.output_dir / "water_network_map.html"
            water_map.save(str(map_path))
            ...
            print(f"Error visualizing water map: {e}")
            return None
    ...
    def visualize_static_graph(self, G):
        try:
            ...
            png_path = self.output_dir / "water_network_graph.png"

            plt.title("Water Infrastructure Network")
            ...
            print(f"Error visualizing water graph: {e}")
            return None

    # =====================================
    # COMPLETE GEOJSON PIPELINE
    # =====================================
    def process_water_network(self, geojson_path):
...
def main():
    ...
    file_path = input("Enter water GeoJSON file path: ").strip()

    result = builder.process_water_network(file_path)
